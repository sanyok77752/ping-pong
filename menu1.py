...
BUTTONS = ['start', 'settings']
    ...
    def draw(self, screen, selected = False):
        ...
        rop_right = border_radius of self.round_top else 0
...
def menu_loop(screen_width, screen_height, screen, settings):
    ...
    for i, text in enumerate(BUTTONS):
        ...
        buttons.append(Button(text, font_obj, buttun_width, button_height, (x,y), round_top,round))
...
def start_menu(WIDTH, HEIGHT, screen):
    settings = Settings()
    menu_loop(WIDTH,HEIGTH, screen, settings)
    return settings
